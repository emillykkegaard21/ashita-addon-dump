...
-- Menus that should hide the minimap (kept in sync with statustimers' hide_menus list)
local hide_menus = T{
...
minimapcontrol.update_menu_state = function()
    ...
    -- Custom hide list (kept in sync with statustimers)
    minimapcontrol.is_hide_menu_open = false
    if menu_now ~= '' then
        for _, v in ipairs(hide_menus) do
            if menu_now:match(v) then
                minimapcontrol.is_hide_menu_open = true
                break
            end
        end
    end
...
minimapcontrol.update_visiblity = function()
    ...
    -- Hide for the custom menu list (kept in sync with statustimers)
    if minimapcontrol.is_hide_menu_open then
        minimapcontrol.visible = false
        return
    end
