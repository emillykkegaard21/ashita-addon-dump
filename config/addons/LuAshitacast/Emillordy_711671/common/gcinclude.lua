...
gcinclude.settings = {
	...
	WSdistance = 7; --default max distance (yalms) to allow non-ranged WS to go off at if the above WScheck is true
	...
	Tele_Ring = 'Dim. Ring (Holla)'; -- put your tele ring in here
};
...
gcinclude.AliasList = T{'gcmessages','wsdistance','setcycle','dt','th','kite','meleeset','gcdrain','gcaspir','nukeset','burst','weapon','elecycle','helix','weather','nuke','death','fight','sir','tankset','proc','cj','pupmode','tpgun','cormsg','forcestring','siphon','warp','tele','rrset','craftset','zeniset','fishset'};
...
function gcinclude.CheckDefault()
	...
	if (gcinclude.RRSET == true) then gFunc.EquipSet(gcinclude.sets.Reraise) end
		...
		local player = gData.GetPlayer();
	if (player.Status ~= 'Engaged') and (gcdisplay.GetToggle('Fight') == true) then
